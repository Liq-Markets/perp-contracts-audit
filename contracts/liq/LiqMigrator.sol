...
import "./interfaces/ILiqIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/ILiqMigrator.sol";

contract LiqMigrator is ReentrancyGuard, ILiqMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "LiqMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "LiqMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "LiqMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "LiqMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "LiqMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "LiqMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "LiqMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "LiqMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "LiqMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "LiqMigrator: token not whitelisted");
        require(_tokenAmount > 0, "LiqMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "LiqMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(liqPrice);
        require(mintAmount > 0, "LiqMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "LiqMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "LiqMigrator: invalid tokenA");
            require(tokenB != address(0), "LiqMigrator: invalid tokenB");
        ...
        ILiqIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "LiqMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "LiqMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "LiqMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "LiqMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("LiqMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "LiqMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "LiqMigrator: invalid _action");

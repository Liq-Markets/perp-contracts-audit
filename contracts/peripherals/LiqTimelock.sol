...
import "./interfaces/ILiqTimelock.sol";
...
contract LiqTimelock is ILiqTimelock {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "LiqTimelock: forbidden");
        _;
    }

    modifier onlyAdminOrHandler() {
        require(msg.sender == admin || isHandler[msg.sender], "LiqTimelock: forbidden");
        _;
    }

    modifier onlyTokenManager() {
        require(msg.sender == tokenManager, "LiqTimelock: forbidden");
        _;
    }

    modifier onlyRewardManager() {
        require(msg.sender == rewardManager, "LiqTimelock: forbidden");
        _;
    }
    ...
    constructor(
        ...
        require(_buffer <= MAX_BUFFER, "LiqTimelock: invalid _buffer");
        require(_longBuffer <= MAX_BUFFER, "LiqTimelock: invalid _longBuffer");
        admin = _admin;
        buffer = _buffer;
    ...
    function setExternalAdmin(address _target, address _admin) external onlyAdmin {
        require(_target != address(this), "LiqTimelock: invalid _target");
    ...
    function setBuffer(uint256 _buffer) external onlyAdmin {
        require(_buffer <= MAX_BUFFER, "LiqTimelock: invalid _buffer");
        require(_buffer > buffer, "LiqTimelock: buffer cannot be decreased");
        buffer = _buffer;
    }

    function setMaxLeverage(address _vault, uint256 _maxLeverage) external onlyAdmin {
      require(_maxLeverage > MAX_LEVERAGE_VALIDATION, "LiqTimelock: invalid _maxLeverage");
    ...
    function setFundingRate(address _vault, uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external onlyAdmin {
        require(_fundingRateFactor < MAX_FUNDING_RATE_FACTOR, "LiqTimelock: invalid _fundingRateFactor");
        require(_stableFundingRateFactor < MAX_FUNDING_RATE_FACTOR, "LiqTimelock: invalid _stableFundingRateFactor");
    ...
    function setFees(
        ...
        require(_taxBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _taxBasisPoints");
        require(_stableTaxBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _stableTaxBasisPoints");
        require(_mintBurnFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _mintBurnFeeBasisPoints");
        require(_swapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _swapFeeBasisPoints");
        require(_stableSwapFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _stableSwapFeeBasisPoints");
        require(_marginFeeBasisPoints < MAX_FEE_BASIS_POINTS, "LiqTimelock: invalid _marginFeeBasisPoints");
        require(_liquidationFeeUsd < 10 * PRICE_PRECISION, "LiqTimelock: invalid _liquidationFeeUsd");
    ...
    function setTokenConfig(
        ...
        require(_minProfitBps <= 500, "LiqTimelock: invalid _minProfitBps");

        IVault vault = IVault(_vault);
        require(vault.whitelistedTokens(_token), "LiqTimelock: token not yet whitelisted");
    ...
    function setInPrivateTransferMode(address _token, bool _inPrivateTransferMode) external onlyAdmin {
        if (excludedTokens[_token]) {
            ...
            require(_inPrivateTransferMode == false, "LiqTimelock: invalid _inPrivateTransferMode");
        }
    ...
    function _mint(address _token, address _receiver, uint256 _amount) private {
        ...
        require(IERC20(_token).totalSupply() <= maxTokenSupply, "LiqTimelock: maxTokenSupply exceeded");
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action] != 0, "LiqTimelock: action not signalled");
        require(pendingActions[_action] < block.timestamp, "LiqTimelock: action time not yet passed");
    }

    function _clearAction(bytes32 _action) private {
        require(pendingActions[_action] != 0, "LiqTimelock: invalid _action");

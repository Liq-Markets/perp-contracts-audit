...
contract LlpManager is ReentrancyGuard, Governable, ILlpManager {
    ...
    constructor(address _vault, address _usdl, address _llp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdl = _usdl;
        llp = _llp;
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdl, uint256 _minLlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("LlpManager: action not enabled"); }
        return _addLiquidity(msg.sender, msg.sender, _token, _amount, _minUsdl, _minLlp);
    }

    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdl, uint256 _minLlp) external override nonReentrant returns (uint256) {
        _validateHandler();
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdl, uint256 _minLlp) private returns (uint256) {
        ...
        uint256 mintAmount = aumInUsdl == 0 ? usdlAmount : usdlAmount.mul(llpSupply).div(aumInUsdl);
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _llpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "LlpManager: cooldown duration not yet passed");

        // calculate aum before sellUSDL
        uint256 aumInUsdl = getAumInUsdl(false);
        uint256 llpSupply = IERC20(llp).totalSupply();

        uint256 usdlAmount = _llpAmount.mul(aumInUsdl).div(llpSupply);
